...
#define FUSED 1
    ...
    //         if (cb.plot_freq)
    //         {
    //             MPI_Gatherv(E_rank + (stride_rankX + 2), stride_rankY *
    //             (stride_rankX + 2), MPI_DOUBLE,
    ...
    //                 for (int rankIter = 0; rankIter < world_size; rankIter++)
    //                 {
    //                     int strideX = (rankIter % cb.px < numSmallRanksX) ?
    //                     smallStrideX : bigStrideX; int strideY = (rankIter /
    //                     cb.px < numSmallRanksY) ? smallStrideY : bigStrideY;
    //                     int offset = sourceOffsets[rankIter];
    //                     for (i = 0; i < strideY; i++)
    //                     {
    //                         for (j = 1; j <= strideX; j++)
    //                         {
    //                             E_prev[offset + i * (n + 2) + (j - 1)] =
    //                             E_prevPacked[rankIter * (bigStrideX *
    //                             bigStrideY + 2 * bigStrideY) + i * (strideX +
    //                             2) + j];
    //                         }
    //                     }
    //                 }
